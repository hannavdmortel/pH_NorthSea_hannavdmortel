...
ax.plot(grouped1.datenum, grouped1.phosphate_trend, c='xkcd:light orange', linewidth=2.2, label='Wadden Sea', alpha=0.9)
ax.plot(grouped2.datenum, grouped2.phosphate_trend, c='royalblue', linewidth=2.2, label='Nearshore (<20 km)', alpha=0.9)
ax.plot(grouped3.datenum, grouped3.phosphate_trend, c='xkcd:teal', linewidth=2.2, label='Intermediate (20-50 km)', alpha=0.9)
ax.plot(grouped4.datenum, grouped4.phosphate_trend, c='xkcd:pink', linewidth=2.2, label='Offshore (≥70 km)', alpha=0.9)
#ax.scatter(df5.YEAR, df5.pH, alpha=0.2, s=8, c='grey', edgecolor='none')
sns.regplot(x=df5.datenum, y=df5.phosphate, ax=ax, fit_reg = False, 
...
#ax.set_ylim([0, 4])
ax.set_ylim(10**-1.2,10**0.8)
ax.set_yscale('log')
ax.set_xlabel('Years')
ax.set_ylabel('DIP (\u03BCmol $\mathregular{L^{-1}}$)')

fig.suptitle('DIP trends Dutch coastal zone')
fig.legend(loc='upper left', bbox_to_anchor=(0.16, -0.02), fontsize=8, ncol=2)
plt.savefig("figures/DIP trend together log.png", bbox_inches='tight')
